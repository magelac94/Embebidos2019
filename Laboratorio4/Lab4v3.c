...
#define OS_MAX_EVENTS			5		// MAX_TCP_SOCKET_BUFFERS + 3 semaforos (Semaforos, queues y mailboxes)
#define STACK_CNT_256			2		// tarea_Led_Red + idle
#define STACK_CNT_512			3		// main() + Interfaz Consola + ejecutar eventos
...
void programaPrincipal(enum tipoUI interfazAUsar, struct tm *pFechaHora, char buffer, Event *punEvento, int pint_opcion_menu, int *pint_id_evento, int *pint_Analog_Value, INT8U *pError ){
    ...
    while(1){ // LOOP menu
		...
		MENU_mostrarMenuPrincipal( interfazAUsar, buffer, NULL );
		MENU_obtenerOpcion( interfazAUsar, buffer, NULL );

		// Nos aseguramos que tenemos el recurso para nosotros
		OSSemPend(pSemaforoAtoI, 0, pError);
		int_opcion_menu = atoi( buffer );
		// Devolvemos el recurso
		OSSemPost(pSemaforoAtoI);

		// Evaluacion de la Opcion
		switch( *pint_opcion_menu ){
			case( OPCION_1 ):
				// CONFIGURAR FECHA HORA
				MENU_pedirFechaHora( interfazAUsar, pFechaHora, buffer, NULL );
				RTC_fijarFechaHora( pFechaHora );
				MENU_printFechaHora( interfazAUsar, pFechaHora, buffer, NULL );		// Imprimo la Fecha y hora modificadoS
				break;

			case( OPCION_2 ):
				// CONSULTAR FECHA HORA ACTUAL
				MENU_consultarHora( interfazAUsar, buffer, NULL );
				RTC_leerFechaHora( pFechaHora );	// Leo el RTC
				MENU_printFechaHora( interfazAUsar, pFechaHora, buffer, NULL ); // Imprimo la Fecha y hora
				break;

			case( OPCION_3 ):
				// AGREGAR EVENTO
				MENU_pedirDatosEvento( interfazAUsar, punEvento, pFechaHora, buffer, NULL );
				EVENTOS_agregarEvento( punEvento );
				break;
			case ( OPCION_4 ):
				// ELIMINAR EVENTO SEGUN EL NUMERO DE EVENTO (ES DE 1 EN adelante segun posicion en el array))
				int_id_evento = MENU_eliminarEvento( interfazAUsar, buffer, NULL );
				printf("%d\n", int_id_evento);
				EVENTOS_eliminarEvento( int_id_evento );  // revisar despues de agregarEvento
				break;
			case( OPCION_5 ):
				// CONSULTAR EVENTO
				MENU_consultarEventos( interfazAUsar, buffer, NULL );
				MENU_printEventos( interfazAUsar, buffer, NULL );
				break;

			case( OPCION_6 ):
				// CONSULTA ANALOGICA
				MENU_pedirEntradaAnalogica( interfazAUsar, buffer, NULL );
				...
				MENU_mostrarEntradaAnalogica( interfazAUsar, &int_Analog_Value, buffer, NULL );
				break;
			case( OPCION_7 ):
				// SALIR
				printf("Salir");
...
void tarea_interfaz_consola(void* pdata){
	...
	programaPrincipal( CONSOLA, &FechaHora, buffer, &unEvento, &int_opcion_menu, &int_id_evento, &int_Analog_Value);
}
...
pSemaforoEventos = OSSemCreate(1);
	...
	Error = OSTaskCreate(tarea_ejecutar_eventos, NULL, 512, 6);
